...
const toggleVisibility = (current: string[] | null, options: string[], option: string) => {
  if (options.length === 0) return current ?? []
...
export function useSkillVisibility(
  ...
  const visibleGroupSet = useMemo(() => new Set(visibleGroupOptions), [visibleGroupOptions])
  const visibleSeriesSet = useMemo(() => new Set(visibleSeriesOptions), [visibleSeriesOptions])

  const hiddenGroupCount = Math.max(0, groupOptions.length - visibleGroupOptions.length)
  const hiddenSeriesCount = Math.max(0, seriesOptions.length - visibleSeriesOptions.length)
