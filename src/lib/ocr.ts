...
export const matchSkillFromText = (
  text: string,
  options: string[],
): string => {
  ...
  for (const option of options) {
    const variants = new Set([option, getSkillLabel(option, 'en'), getSkillLabel(option, 'ja')])
    ...
    if (bestOptionScore > bestScore.score) {
      bestScore = { skill: option, length: bestOptionLength, score: bestOptionScore }
    } else if (bestOptionScore === bestScore.score && bestOptionLength > bestScore.length) {
      bestScore = { skill: option, length: bestOptionLength, score: bestOptionScore }
    }

    if (bestOptionMonoScore > bestMonoScore.score) {
      bestMonoScore = { skill: option, length: bestOptionMonoLength, score: bestOptionMonoScore }
    } else if (
...
const binarizeWithWebGpu = async (canvas: HTMLCanvasElement, threshold: number) => {
  ...
  const gpuMapMode = (globalThis as { GPUMapMode?: Record<string, number> }).GPUMapMode
