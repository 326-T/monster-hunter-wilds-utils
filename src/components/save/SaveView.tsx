...
export function SaveView({
	...
	const pickDefaultSkill = useCallback(
		(hiddenCount: number, options: string[]) => {
	...
	}, [hiddenSeriesCount, pickDefaultSkill, seriesOptions]);

	const defaultGroupSkill = useMemo(() => {
		if (groupOptions.length === 0) return "";
		return pickDefaultSkill(hiddenGroupCount, groupOptions);
	}, [groupOptions, hiddenGroupCount, pickDefaultSkill]);
	...
	const handleAddEntry = (event: React.FormEvent<HTMLFormElement>) => {
		...
		if (!selectedTableKey || !resolvedGroupSkill || !resolvedSeriesSkill)
			return;
						...
						>
							<div className="flex flex-col gap-2">
								...
								<div className="flex flex-wrap gap-2 text-xs text-muted-foreground">
									<span>
										{t("save.cursorPosition", { value: currentCursor })}
									</span>
									<span>
										{t("save.entryCount", { count: sortedEntries.length })}
									</span>
								</div>
							</div>
						</div>
											...
											<div className="grid gap-4 lg:grid-cols-2">
												<div className="grid gap-3 rounded-2xl border border-border/40 bg-background p-4">
													...
													<div className="flex flex-wrap gap-2 text-xs text-muted-foreground">
														<span>
															{t("common.visible", {
																count: visibleSeriesOptions.length,
															})}
														</span>
														<span>
															{t("common.total", {
													...
													<div className="max-h-40 space-y-2 overflow-y-auto">
														...
														{seriesOptions.map((option, index) => {
															const inputId = `series-visible-${index}`;
															return (
																<label
																	key={option}
																	htmlFor={inputId}
												...
												<div className="grid gap-3 rounded-2xl border border-border/40 bg-background p-4">
													...
													<div className="flex flex-wrap gap-2 text-xs text-muted-foreground">
														<span>
															{t("common.visible", {
																count: visibleGroupOptions.length,
															})}
														</span>
														<span>
															{t("common.total", {
													...
													<div className="max-h-40 space-y-2 overflow-y-auto">
														...
														{groupOptions.map((option, index) => {
															const inputId = `group-visible-${index}`;
															return (
																<label
																	key={option}
																	htmlFor={inputId}
							...
							<table className="w-full border-collapse text-sm">
								...
								<tbody>
									...
									{visibleEntries.map((entry) => {
										...
										const seriesValue = seriesSelectOptions.includes(
											entry.seriesSkill,
										)
											? entry.seriesSkill
											: HIDDEN_SKILL_LABEL;
										const groupValue = groupSelectOptions.includes(
											entry.groupSkill,
										)
											? entry.groupSkill
											: HIDDEN_SKILL_LABEL;
